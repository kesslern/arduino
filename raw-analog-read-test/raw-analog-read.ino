 ...
 * to reduce bouncing.
 * 
 * Attach the analog read pin to the analog input source of interest. 
 *
 * Readings will be published when the button is pressed.
 * 
 **/
...
void loop() {
  ...
  if (buttonStateChanged && buttonState == DOWN) {
    Particle.publish("analog read", String(analogRead(A0)).c_str());
  }
}
